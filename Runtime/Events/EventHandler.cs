...
namespace XRC.Core.Events
{
    ...
    public class EventHandler : MonoBehaviour
    {
            ...
            if (s_GlobalEventTable.TryGetValue(eventName, out actionList)) {
                ...
                for (int i = 0; i < actionList.Count; ++i) {
                    if (actionList[i].GetDelegate() == invokableAction.GetDelegate()) {
                        Debug.LogWarning("Warning: the function \"" + invokableAction.GetDelegate().Method + 
        ...
        {
            var invokableAction = GenericObjectPool.Get<InvokableAction>();
        ...
        {
            var invokableAction = GenericObjectPool.Get<InvokableAction<T1>>();
            invokableAction.Initialize(action);
            RegisterEvent(eventName, invokableAction);
        }
        ...
        {
            var invokableAction = GenericObjectPool.Get<InvokableAction<T1, T2>>();
            invokableAction.Initialize(action);
            RegisterEvent(eventName, invokableAction);
        }
        ...
        {
            var invokableAction = GenericObjectPool.Get<InvokableAction<T1, T2, T3>>();
            invokableAction.Initialize(action);
            RegisterEvent(eventName, invokableAction);
        }
        ...
        {
            var invokableAction = GenericObjectPool.Get<InvokableAction<T1, T2, T3, T4>>();
            invokableAction.Initialize(action);
            RegisterEvent(eventName, invokableAction);
        }
        ...
        {
            var invokableAction = GenericObjectPool.Get<InvokableAction<T1, T2, T3, T4, T5>>();
            invokableAction.Initialize(action);
            RegisterEvent(eventName, invokableAction);
        }
        ...
        {
            var invokableAction = GenericObjectPool.Get<InvokableAction<T1, T2, T3, T4, T5, T6>>();
            invokableAction.Initialize(action);
            RegisterEvent(eventName, invokableAction);
        }
        ...
        public static void ExecuteEvent<T1, T2, T3, T4, T5, T6>(string eventName, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6)
        {
            ...
            if (actions != null) {
                for (int i = actions.Count - 1; i >= 0; --i) {
                    (actions[i] as InvokableAction<T1, T2, T3, T4, T5, T6>).Invoke(arg1, arg2, arg3, arg4, arg5, arg6);
                }
            }
        }
            ...
            if (actions != null) {
                for (int i = 0; i < actions.Count; ++i) {
                    ...
                    if (invokeableAction.IsAction(action)) {
                        GenericObjectPool.Return(invokeableAction);
                        actions.RemoveAt(i);
                        break;
                    }
                }
